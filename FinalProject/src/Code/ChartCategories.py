...
def generatePieChart(size_list, attack_list):
    ...
    plt.title("Trending Cybersecurity Categories on Twitter (per 50,000 tweets)")
    ax.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    plt.savefig(str(ff.PATHS['CHART_PATH'] + "PieChartRepCat4.PNG"))
    #plt.show()
    plt.close()

# generate a bar chart
def generateBarChart(size_list, attack_list):
    y_pos = np.arange(len(size_list))
    colors = []
    for c in range(len(size_list)):
        colors.append(np.random.rand(3, ))
    fig = plt.figure()
    ...
    #
    #plt.show()
    plt.savefig(str(ff.PATHS['CHART_PATH'] + "BarChartRepCat4.PNG"))
    plt.close()